...
const gridOptions: VxeTableGridOptions<AgencyRegisterItem> = {
  ...
  checkboxConfig: {
    checkMethod: ({ row }) => Number(row.Approve ?? row.Status) === 1,
  },
  proxyConfig: {
    ajax: {
      query: async ({ page }) => {
...
const [Grid, gridApi] = useVbenVxeGrid({
  gridEvents: {
    checkboxAll: ({ records }: { records: AgencyRegisterItem[] }) =>
      (selectedRows.value = records),
    checkboxChange: ({ records }: { records: AgencyRegisterItem[] }) =>
      (selectedRows.value = records),
  },
  gridOptions,
});
  ...
  <div v-if="canView">
    <div class="ops-query-scope mb-3">
      <div class="ops-query-filters">
        ...
        <div class="flex flex-col gap-1">
          <Input v-model:value="email" allow-clear placeholder="请输入申请邮箱">
            <template #addonBefore>申请邮箱</template>
          </Input>
        </div>
        <div class="flex flex-col gap-1">
          <Input v-model:value="regIp" allow-clear placeholder="请输入注册 IP">
            <template #addonBefore>注册 IP</template>
          </Input>
        </div>
        <div class="flex flex-col gap-1">
          <Input v-model:value="mobile" allow-clear placeholder="请输入手机号">
...
批量拒绝
      ...
      <Space v-if="canAutoAudit">
        ...
        <Switch
          :checked="autoAudit"
          @change="(value) => toggleAutoAudit(!!value)"
        />
      </Space>
    </div>
    <Grid>
      <template #status="{ row }">
        <Tag
          :color="
            Number(row.Approve ?? row.Status) === 2
              ? 'success'
              : Number(row.Approve ?? row.Status) === 3
                ? 'error'
                : 'default'
          "
        >
