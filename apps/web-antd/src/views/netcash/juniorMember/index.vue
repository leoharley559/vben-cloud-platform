...
function writeWorkbook(data: Row[], filename: string) {
...
watch(activeTab, (value) => {
  if (value === 'change' && recordRows.value.length === 0) loadRecords();
});

onMounted(async () => {
  activeTab.value = canViewMember.value ? 'member' : 'change';
  await loadOptions();
  if (canViewMember.value && canViewTable.value) loadMembers();
  if (!canViewMember.value && canViewChange.value) loadRecords();
});
</script>
...
<template>
  ...
  <Result v-else status="403" sub-title="无下级成员页面权限" title="403" />
</template>
