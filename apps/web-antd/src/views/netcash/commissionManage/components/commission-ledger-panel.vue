...
const isMulti = computed(() =>
  props.variant === 'multi-single'
    ? 1
    : (props.variant === 'multi-multi'
      ? 2
      : 0),
);
...
const canExport = computed(() =>
  checkPermission(isTeam.value ? 10_477 : 10_471),
);
const canBatch = computed(
  () =>
    props.context === 'grant' &&
    checkPermission(isTeam.value ? 10_476 : 10_470),
);
const canSend = computed(
  () =>
    props.context === 'grant' &&
    checkPermission(isTeam.value ? 10_478 : 10_472),
);
const canAdjust = computed(
  () =>
    props.context === 'grant' &&
    checkPermission(isTeam.value ? 10_482 : 10_473),
);
const canViewDetail = computed(
  () =>
    props.context === 'grant' &&
    checkPermission(isTeam.value ? 10_485 : 10_474),
);
...
const summaryItems = computed(() => [
  {
    label: '总输赢',
    value: cent(
      Number(total.value.SumBetGold || 0) - Number(total.value.SumWinGold || 0),
    ),
  },
  { label: '平台费', value: cent(total.value.SumApiFeeTotal) },
  {
    ...
    value: cent(
      total.value.SumCommissionChangeAfter ?? total.value.SumCommissionTotal,
    ),
  },
  ...
  return {
    ...
    AmountMax:
      payload.AmountMax === undefined
        ? ''
        : Math.round(Number(payload.AmountMax) * 100),
    AmountMin:
      payload.AmountMin === undefined
        ? ''
        : Math.round(Number(payload.AmountMin) * 100),
    ...
    SettlementBeginTime:
      commissionRange.value?.[0]?.startOf('day').unix() || '',
...
function canSelectRow(row: Record<string, any>) {
  ...
  return (
    end > 0 && dayjs().isBefore(dayjs.unix(end).subtract(1, 'millisecond'))
  );
}
...
function openGrant(row?: Record<string, any>) {
  ...
  const hasDeduction = selected.some(
    (item) => Number(item.Deduction || 0) !== 0,
  );
  let deduct = false;
  Modal.confirm({
    content: () =>
...
function openAdjust(row: Record<string, any>) {
  ...
  Object.assign(adjustForm, {
    ...
    CommissionTotal: cent(
      Number(row.CommissionChangeAfter) > 0
        ? row.CommissionChangeAfter
        : row.CommissionTotal,
    ),
    Desc: '',
    Id: row.Id,
...
async function submitAdjust() {
  ...
  try {
    await adjustCommissionApi({
      ...adjustForm,
      CommissionChangeAmount: Math.round(
        Number(adjustForm.CommissionChangeAmount) * 100,
      ),
    });
  ...
  try {
    ...
    detailRows.value = normalizeRows(result).map((item) => ({
      ...
      ApiFeeTotal:
        item.ApiFeeTotal ??
        Math.floor(
          (Number(item.SumWinLoseGold || 0) * Number(item.Fee || 0)) / 100,
        ),
      GameName: formatVenueName(item.GameType, gameConfig.value),
    }));
    if (isTeam.value) {
      detailTotal.ActivityUserNum = detailRows.value.reduce(
        (sum, item) => sum + Number(item.ActivityUserNum || 0),
        0,
      );
      detailTotal.WinLoss = detailRows.value.reduce(
        (sum, item) =>
          sum + Number(item.BetGold || 0) - Number(item.WinGold || 0),
        0,
      );
      detailTotal.RedGold = detailRows.value.reduce(
        (sum, item) => sum + Number(item.RedGold || 0),
        0,
      );
      detailTotal.BackWaterGold = detailRows.value.reduce(
        (sum, item) => sum + Number(item.BackWaterGold || 0),
        0,
      );
    } else {
      detailTotal.SumBetGold = detailRows.value.reduce(
        (sum, item) => sum + Number(item.SumBetGold || 0),
        0,
      );
      detailTotal.SumWinLoseGold = detailRows.value.reduce(
        (sum, item) => sum + Number(item.SumWinLoseGold || 0),
        0,
      );
      detailTotal.ApiFeeTotal = detailRows.value.reduce(
        (sum, item) => sum + Number(item.ApiFeeTotal || 0),
        0,
      );
    }
  } catch {
...
function displayCell(
  ...
  switch (key) {
    ...
    case 'AgentStatus': {
      return Number(row.AgentStatus) === 1
        ? '启用'
        : (Number(row.AgentStatus) === 2
          ? '停用'
          : '-');
    }
    case 'CommissionRate': {
    ...
    case 'CommissionTotal': {
      const amount =
        Number(row.CommissionChangeAfter || 0) > 0
          ? row.CommissionChangeAfter
          : row.CommissionTotal;
      let prefix = '';
      if (isMultiRate.value && Number(row.CommissionType) === 1)
        prefix = '场馆佣金：';
      if (isMultiRate.value && Number(row.CommissionType) === 2)
        prefix = '流水佣金：';
    ...
    case 'IsMeetSettlementReq': {
      return Number(row.IsMeetSettlementReq) === 1
        ? '是'
        : (Number(row.IsMeetSettlementReq) === 2
          ? '否'
          : '全部');
    }
    case 'IsSettlement': {
    ...
    case 'ReversalWinLoss': {
      return cent(
        Number(row.CleanBetWinTotal || 0) +
          Number(row.LastMonthCleanBetWinTotal || 0),
      );
    }
    case 'SettlementDate': {
    ...
    case 'WithdrawPayFee': {
      return cent(
        Number(row.WithdrawMoneyFee || 0) + Number(row.PayMoneyFee || 0),
      );
    }
    default: {
      return row[key] ?? '-';
...
function displayTotalCell(key: string): number | string {
  ...
  switch (key) {
    case 'ActivityUserNum': {
      return Number(sum.SumActivityUserNum || 0);
    }
    case 'AdminChargeMoney': {
      return cent(sum.SumAdminChargeMoney);
    }
    case 'ApiFeeTotal':
    case 'BackWaterGold':
    case 'CleanBetWinTotal':
    ...
    case 'ReversalWinLoss': {
      return cent(
        Number(sum.SumCleanBetWinTotal || 0) +
          Number(sum.SumLastMonthCleanBetWinTotal || 0),
      );
    }
    case 'TeamCount': {
      return Number(sum.SumDeputyCount || 0) + Number(sum.SumMainCount || 0);
    }
    case 'WinLoss': {
      return cent(Number(sum.SumBetGold || 0) - Number(sum.SumWinGold || 0));
    }
    case 'WithdrawPayFee': {
      return cent(
        Number(sum.SumWithdrawMoneyFee || 0) + Number(sum.SumPayMoneyFee || 0),
      );
    }
    default: {
      return '-';
...
<Input v-model:value="query.TeamName" allow-clear />
</Form.Item>
          </Col>
...
<Input v-model:value="query.AgentAccount" allow-clear />
          ...
          <Col :lg="4" :md="6" :sm="12" :xs="24">
            <Form.Item label="佣金月份">
<DatePicker v-model:value="month" picker="month" class="w-full" />
</Form.Item>
          </Col>
          <Col :lg="4" :md="6" :sm="12" :xs="24">
          ...
          <Col :lg="4" :md="6" :sm="12" :xs="24">
            <Form.Item label="发展人">
<Input v-model:value="query.DeveloperName" allow-clear />
</Form.Item>
          </Col>
          <Col :lg="4" :md="6" :sm="12" :xs="24">
            <Form.Item label="维护人">
<Input v-model:value="query.MaintainerName" allow-clear />
</Form.Item>
          </Col>
          <Col v-if="!isTeam" :lg="4" :md="6" :sm="12" :xs="24">
            <Form.Item label="是否停用">
              <Select
                v-model:value="query.AgentStatus"
                allow-clear
                :options="[
                  { label: '启用', value: 1 },
                  { label: '停用', value: 2 },
          ...
          <Col :lg="4" :md="6" :sm="12" :xs="24">
            <Form.Item label="上月是否有结余">
              <Select
                v-model:value="query.IsLastMonthBetWin"
                :options="[
                  { label: '全部', value: 0 },
                  { label: '是', value: 1 },
                  { label: '否', value: 2 },
...
<InputNumber
          ...
          <Col :lg="4" :md="6" :sm="12" :xs="24">
            <Form.Item label="活跃人数最小">
<InputNumber
                v-model:value="query.ActivityUserNumMin"
                :min="0"
                class="w-full"
            />
</Form.Item>
          ...
          <Col :lg="4" :md="6" :sm="12" :xs="24">
            <Form.Item label="活跃人数最大">
<InputNumber
                v-model:value="query.ActivityUserNumMax"
                :min="0"
                class="w-full"
            />
</Form.Item>
          ...
          <Col
            v-if="isMultiMode && context === 'grant'"
            :lg="4"
            :md="6"
            :sm="12"
            :xs="24"
          >
            <Form.Item label="达到派发条件">
              <Select
                v-model:value="query.IsMeetSettlementReq"
                :options="[
                  { label: '全部', value: 0 },
                  { label: '是', value: 1 },
                  { label: '否', value: 2 },
          ...
          <Col v-if="isMultiRate" :lg="4" :md="6" :sm="12" :xs="24">
            <Form.Item label="佣金类型">
              <Select
                v-model:value="query.CommissionType"
                allow-clear
                :options="[
                  { label: '场馆佣金', value: 1 },
                  { label: '流水佣金', value: 2 },
          ...
          <Col v-if="context === 'record'" :lg="4" :md="6" :sm="12" :xs="24">
            <Form.Item label="发放人">
<Input v-model:value="query.SettlementName" allow-clear />
</Form.Item>
          </Col>
          <Col :lg="4" :md="6" :sm="12" :xs="24">
            <Form.Item label="上级账号">
<Input v-model:value="query.MainUsername" allow-clear />
</Form.Item>
          </Col>
          <Col :lg="4" :md="6" :sm="12" :xs="24">
            <Form.Item label="备注">
<Input v-model:value="query.Desc" allow-clear />
</Form.Item>
          </Col>
          <Col :lg="4" :md="6" :sm="12" :xs="24">
...
<DatePicker.RangePicker
          ...
          <Col :lg="8" :md="12" :sm="24" :xs="24">
            <Form.Item label="佣金日期">
<DatePicker.RangePicker
                v-model:value="commissionRange"
                class="w-full"
            />
</Form.Item>
          </Col>
...
查询
    ...
    <Alert
      v-if="context === 'grant'"
      class="mb-4"
      message="手动发放只允许发放已结束结算周期且未结算的记录。"
      type="warning"
      show-icon
    />
      ...
      >
        ...
        <template #expandedRowRender="{ record }">
          <Table
            ...
            :columns="
              allColumns.filter(
                (column) =>
                  !['actions', 'TeamName', 'TeamCount'].includes(
                    columnKey(column),
                  ),
              )
            "
        ...
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'Type'">
            <Tag :color="Number(record.Type) === 2 ? 'red' : 'blue'">
{{
              displayCell('Type', record)
            }}
</Tag>
          </template>
          <template v-else-if="column.key === 'AgentStatus'">
            <Tag :color="Number(record.AgentStatus) === 1 ? 'green' : 'red'">
{{
              displayCell('AgentStatus', record)
            }}
</Tag>
          </template>
          <template v-else-if="column.key === 'IsSettlement'">
            <Tag
              :color="Number(record.IsSettlement) === 1 ? 'green' : 'orange'"
              >
{{ displayCell('IsSettlement', record) }}
</Tag>
          </template>
          <template v-else-if="column.key === 'ApiFeeTotal' && canViewDetail">
            <Button type="link" size="small" @click="openDetail(record)">
{{
              displayCell('ApiFeeTotal', record)
            }}
</Button>
          </template>
          <template v-else-if="column.key === 'CommissionRate' && isMultiRate">
            <Button
              type="link"
              size="small"
              @click="openRates(record.CommissionRate)"
              >
查看
</Button>
          </template>
            ...
            <span
              :class="{
                negative:
                  ...
                  Number(
                    String(displayCell(columnKey(column), record)).replaceAll(
                      ',',
                      '',
                    ),
                  ) < 0,
          ...
          <Table.Summary fixed>
            <Table.Summary.Row>
              ...
              >
                ...
                <template v-else>
{{
                  displayTotalCell(columnKey(column))
                }}
</template>
              </Table.Summary.Cell>
            </Table.Summary.Row>
...
<Input v-model:value="adjustForm.ReportMonth" disabled />
</Form.Item>
        <Form.Item label="佣金余额">
<Input v-model:value="adjustForm.CommissionTotal" disabled />
</Form.Item>
        <Form.Item
          label="调整金额"
          name="CommissionChangeAmount"
          :rules="[{ required: true, message: '请输入调整金额' }]"
        >
          <InputNumber
            v-model:value="adjustForm.CommissionChangeAmount"
            :precision="2"
            class="w-full"
          />
        </Form.Item>
        <Form.Item label="调整原因">
<Input v-model:value="adjustForm.Remark" :maxlength="200" />
</Form.Item>
        <Form.Item label="备注">
<Input.TextArea v-model:value="adjustForm.Desc" :maxlength="500" />
</Form.Item>
      </Form>
    </Modal>
    ...
    >
      <Descriptions class="mb-3" bordered size="small">
        <template v-if="isTeam">
          <Descriptions.Item label="下级活跃人数">
{{
            detailTotal.ActivityUserNum || 0
          }}
</Descriptions.Item>
          <Descriptions.Item label="总输赢">
{{
            cent(detailTotal.WinLoss)
          }}
</Descriptions.Item>
          <Descriptions.Item label="红利">
{{
            cent(detailTotal.RedGold)
          }}
</Descriptions.Item>
          <Descriptions.Item label="返水">
{{
            cent(detailTotal.BackWaterGold)
          }}
</Descriptions.Item>
        </template>
        <template v-else>
...
{{
            cent(detailTotal.SumBetGold)
          }}
</Descriptions.Item>
          <Descriptions.Item label="总输赢">
{{
            cent(detailTotal.SumWinLoseGold)
          }}
</Descriptions.Item>
          <Descriptions.Item label="平台费">
{{
            cent(detailTotal.ApiFeeTotal)
          }}
</Descriptions.Item>
        </template>
      </Descriptions>
      ...
      >
        ...
        <Table.Column
          v-if="isTeam"
          data-index="ActivityUserNum"
          title="活跃人数"
        />
        <Table.Column v-else key="SumBetGold" title="流水">
          <template #default="{ record }">
{{
            cent(record.SumBetGold)
          }}
</template>
        </Table.Column>
...
{{
            cent(
              isTeam
                ? Number(record.BetGold || 0) - Number(record.WinGold || 0)
                : record.SumWinLoseGold,
            )
          }}
</template>
        </Table.Column>
    ...
    >
      <Table :data-source="rateRows" :pagination="false" size="small" bordered>
        ...
        <Table.Column key="rateWin" title="输赢分成">
          <template #default="{ record }">
{{ Number(record.WinLoseRate || 0) / 100 }}%
</template>
        </Table.Column>
        <Table.Column key="rateWater" title="流水分成">
          <template #default="{ record }">
{{ Number(record.WaterRate || 0) / 100 }}%
</template>
        </Table.Column>
      </Table>
    </Modal>
